-- Dry-run SQL puro. O corpo da migration é aplicado e revertido nesta transação.
BEGIN;
...
BEGIN
  ...
  SELECT coalesce(jsonb_agg(jsonb_build_object('funcionario_id', f.id, 'nome', f.nome,
    ...
    ORDER BY f.nome), '[]'::jsonb) INTO v_itens FROM public.funcionarios f WHERE f.id = ANY(v_origem);
