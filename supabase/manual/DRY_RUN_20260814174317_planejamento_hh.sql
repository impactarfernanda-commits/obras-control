...
DO $check$
BEGIN
  IF to_regclass('public.funcionario_custos_vigencias') IS NULL THEN
    RAISE EXCEPTION 'Tabela de vigencias ausente';
  END IF;
  IF EXISTS (
    SELECT 1 FROM information_schema.role_table_grants
     WHERE table_schema = 'public'
       AND table_name = 'funcionario_custos_vigencias'
       AND grantee IN ('PUBLIC', 'anon', 'authenticated')
  ) THEN RAISE EXCEPTION 'ACL financeira indevida'; END IF;
  IF NOT (SELECT rowsecurity FROM pg_tables
    WHERE schemaname = 'public' AND tablename = 'funcionario_custos_vigencias') THEN
    RAISE EXCEPTION 'RLS de vigencias ausente';
  END IF;
  IF EXISTS (
    SELECT 1 FROM public.funcionario_custos_vigencias a
    JOIN public.funcionario_custos_vigencias b
      ON a.funcionario_id = b.funcionario_id AND a.id < b.id
  ...
  ) THEN RAISE EXCEPTION 'Vigencias sobrepostas'; END IF;
  IF EXISTS (
    SELECT 1 FROM public.funcionario_custos_vigencias
     WHERE custo_mensal_total <> round(
       salario + encargos_calculados + provisao_13 + provisao_aviso_previo
       + provisao_ferias + assistencia_medica + assistencia_odontologica
       + vale_alimentacao + multibeneficio + seguro_vida, 2)
  ) THEN RAISE EXCEPTION 'Composicao de custo inconsistente'; END IF;
END;
$check$;

-- Exercicios transacionais: nova vigencia, substituicao no mesmo dia, salario,
-- categoria, seguro e beneficio sao cobertos pelos triggers. A consulta por data
-- abaixo comprova a selecao temporal usada pelo servidor.
DO $comportamento$
DECLARE v_funcionario uuid; v_inicio date; v_quantidade integer;
BEGIN
  SELECT funcionario_id, min(vigencia_inicio) INTO v_funcionario, v_inicio
    FROM public.funcionario_custos_vigencias GROUP BY funcionario_id LIMIT 1;
  IF v_funcionario IS NULL THEN RETURN; END IF;
  PERFORM public.registrar_funcionario_custo_vigencia(
    v_funcionario, current_date + 1, 'funcionario', 'apurado_por_vigencia');
  PERFORM public.registrar_funcionario_custo_vigencia(
    v_funcionario, current_date + 1, 'beneficios', 'apurado_por_vigencia');
  SELECT count(*) INTO v_quantidade FROM public.funcionario_custos_vigencias
   WHERE funcionario_id = v_funcionario AND vigencia_inicio = current_date + 1;
  IF v_quantidade <> 1 THEN RAISE EXCEPTION 'Mudanca no mesmo dia criou periodo impossivel'; END IF;
  IF NOT EXISTS (
    SELECT 1 FROM public.funcionario_custos_vigencias
     WHERE funcionario_id = v_funcionario AND v_inicio BETWEEN vigencia_inicio AND COALESCE(vigencia_fim, 'infinity'::date)
  ) THEN RAISE EXCEPTION 'Historico inicial deixou de ser consultavel'; END IF;
END;
$comportamento$;

-- custoDoDia, HE e ausencias sao testados no TypeScript com custos mensais de
-- 8.000 e 8.700; este dry-run valida a fonte temporal e suas protecoes no banco.
ROLLBACK;
