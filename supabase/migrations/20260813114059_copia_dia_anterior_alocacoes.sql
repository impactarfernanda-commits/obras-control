...
BEGIN
  ...
  WHERE f.id = ANY(v_origem)
    AND (f.deleted_at IS NOT NULL OR f.visivel_obras_control IS FALSE
      OR (f.data_admissao IS NOT NULL AND f.data_admissao > p_data_destino)
  ...
  IF p_aplicar THEN
    WITH inseridas AS (
      ...
      EXCEPT
      SELECT unnest(v_existentes), p_obra_id, p_data_destino, v_usuario
      EXCEPT
      SELECT unnest(v_inelegiveis), p_obra_id, p_data_destino, v_usuario
  ...
  FROM public.funcionarios f WHERE f.id = ANY(v_origem);

  RETURN jsonb_build_object(
    'origem_data', p_data_origem, 'destino_data', p_data_destino,
    'total_origem', cardinality(v_origem),
    'total_copiados', cardinality(v_copiados),
